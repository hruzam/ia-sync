...
- `ai/experimental.zsh` — **Experimental-runner dispatcher.** `_exp_list` / `_exp_run` (→ `exp-list` / `exp-run`) discover and execute `ai/experimental/<id>/runner.zsh` lazily. The whole directory deploys through the existing recursive zsh rsync; new experiments do not need a deploy-script edit. The full runner contract is `ai/experimental/README.md`. `ox-alpha` is the first convenience alias, routing to `exp-run ox-alpha`. Sourced by `base.zsh` PARTITION 9. Aliases in `keyboard.zsh` PARTITION 18.
