...
# bluebottle.sh — fast synthesizer
# Model: gemini-2.5-flash (from handoff MODEL ASSIGNMENTS)
# Three modes:
#   no arg  → UI session (interactive gemini — needs TTY, do NOT add < /dev/null)
#   --raw   → headless research pass (REST, no system instruction)
#   with arg → headless synthesis (REST primary, graceful fail on REST failure)
#
...
# Class B hang confirmed: positional-arg invocation in $() hangs immediately.
# CLI and REST share the same backend — CLI cannot rescue a REST 503.
...
if [[ $# -eq 0 ]]; then
    ...
    shift
    _INPUT="$*"
    # Headless research pass (no persona injection)
    if [[ -f "$_INPUT" ]]; then
        _INPUT=$(cat "$_INPUT")
    fi
    ...
    _PERSONA=$(grep -A9999 "^---" ~/.gemini/agents/bluebottle.md 2>/dev/null | tail -n +2)
    _KEY=$(_gai_api_key) || { echo "[UNAVAILABLE: no API key]"; exit 0; }
