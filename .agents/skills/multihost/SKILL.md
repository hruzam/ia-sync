...
I am a **project-local** skill: `~/ia-sync/.Codex/skills/multihost/`. Not global.
...
- **I deliver myself by `git pull`.** I live *inside* the repo, so I am not part of the
  `~/.Codex/skills` sync payload and do not wait on `deploy.sh`. Pull, open a session in
...
The marker lives in `~/.local/state/` deliberately: `sync.sh` covers only `~/.Codex`,
...
|---|---|
| `deploy.sh` zsh + Codex legs are `rsync -a`, **no `--delete`** | Archiving a file on A **cannot** break B — B's live copy survives the pull. This is what makes one-sided retirement safe. |
| `sync.sh` `Codex/skills/` and `Codex/commands/` run **`--delete`** | A sync from a machine that has not pulled+deployed **wipes** the other's contributions. Deploy before sync, always, both directions. |
| `sync.sh` `Codex/agents/` is additive | Agents are the only safe leg. |
