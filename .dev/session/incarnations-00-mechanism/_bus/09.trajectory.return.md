# RETURN — canon-guard probe results (promoted)

`promoted-from: ~/reposoma/_mail/oraculum/inbox/trajectory.canon-guard-probe.2026-09-17.md (archived)`
`promoted-by: oraculum (head) · 2026-09-17 · verbatim below the line`
`world-testimony note (honest limit): the throwaway repo AND its four session transcripts were`
`deleted per the brief's disposability intent — record-level witnessing (cartan-style) is not`
`possible for this probe; the evidence standard is the report itself + reposoma status (one new`
`file, as claimed). Acceptable: this was a deploy-nothing evidence probe, not a trial row. The`
`report's internal quality is high: triple-source schema verification (two doc-agents vs the`
`installed binary's own embedded reference vs live production rule files), the binary trusted over`
`two correlated doc-agents, soft-vs-hard stop distinguished, delivery-observability caveat named,`
`next probe identified instead of overclaimed.`

`HEAD'S READING — the model refinement this probe forces:`
`rows 0/0R measured guidance at BOOT distance (AGENTS/CLAUDE as user message, seed read at t0):`
`zero fired. this probe measured guidance at READ distance (path-scoped rule injected at the`
`file-read adjacent to the act): soft stop, 2/2 fresh sessions, zero priming, rule quoted verbatim`
`unprompted. and enforcement (PreToolUse deny): hard stop, no bypass attempt.`
`⇒ NOT "guidance fails" but a PROXIMITY GRADIENT: instruction effectiveness decays with distance`
`from the decision point; only enforcement is structural. Direct consequence for the seed`
`mechanism: a seed read at boot IS boot-distance guidance — the zero-fired result is the gradient,`
`not a broken bed; trigger-time delivery (consult-at-edge, rule-tier) is where prompt-layer`
`inheritance can plausibly bend hands; hooks are where canon actually gets protected.`

---

*(trajectory's report, verbatim)*
