# RETURN — row 0 boot report (promoted)

`promoted-from: ~/reposoma/_mail/oraculum/inbox/trajectory.vocabulary-edits.2026-09-13.md`
`(inbox copy archived per mail protocol — durable citation is THIS file, never the inbox)`
`promoted-by: oraculum (head) · 2026-09-14 · verbatim below the line`
`world-testimony check: diffs read 2026-09-14 (Delta, read-only) MATCH this report 1:1 —`
`reposoma raw.therapy/README.md dirty, exactly line 23; nabla-lab spec-v2 dirty, exactly the`
`four named spots + naming note. Claim integrity: PASS.`
