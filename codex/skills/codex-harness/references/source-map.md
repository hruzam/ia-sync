# Codex harness source map

Use this as a router, not a mandatory full-read list.

| Source | Authority and use |
|---|---|
| Current official OpenAI documentation through `openai-docs` | Current Codex syntax, discovery, configuration, agents, skills, hooks, rules, models, and client behavior |
| `~/ia-sync/codex/` | Portable global Codex authoring/deploy source; inspect before every global primitive change |
| `~/ia-sync/claude/` | Portable Claude agentive pattern corpus; use for semantic cross-section, never copy mechanically |
...
- Start with the active project's own instructions and evidence.
- Search by concept and failure mode, not only by remembered filename.
- Prefer current portable source over deployed runtime copies.
- Label historical version claims as stale until reverified.
- When two maps disagree, report the curvature and identify which authority can resolve it.
